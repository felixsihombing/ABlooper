...
def mask_run_epoch_refine(model, optim, train_dataloader, val_dataloader, decoys=5, grad_clip=10.0, dist_check_weight=1):
    '''
    Function to train model for a single epoch with mask. Only works with batch size 1
    '''
    CDRs = ['H1', 'H2', 'H3', 'L1', 'L2', 'L3']
    cdr_rmsds = []
    epoch_train_losses = []
    ...
    for i,data in enumerate(train_dataloader):                         # For each batch of data in the dataset
        ...
        loss = rmsd(geomouts, pred) + dist_check_weight * dist_check(pred.mean(0, keepdim = True), node_features)
        epoch_train_losses.append(loss.item())                         # Store value of loss function for training set
...
def train_model_refine(model, optimiser, train_dataloader, val_dataloader, training_name='', n_epochs=5000, patience=150, decoys=5):
    '''
    Runs run_epoch function a specified number of times and keeps track of loss.
    '''
    train_losses = []
