...
class MaskDecoyGen(torch.nn.Module):
    ...
    def forward(self, node_features, coordinates, mask):
        geoms = torch.zeros((self.decoys, *coordinates.shape), device=coordinates.device)

        for i, block in enumerate(self.blocks):
            geoms[i,:,:,:] = block(node_features, coordinates, mask)[1] # only save geoms, array in order: (decoy, batch, nodes, dimensions)
