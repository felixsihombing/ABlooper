...
model.load_state_dict(torch.load('best_models/best_model-0305-Radam-1-2optim-4', map_location=torch.device(device)))

# set optimisers
optimiser = torch.optim.Adam(model.parameters(), lr=1e-4, weight_decay=1e-4)
optimiser.load_state_dict(torch.load('previous/previous_optim-0305-Radam-1-2optim-4', map_location=torch.device(device)))

# Step to actually train the network
train_losses, val_losses = train_model_refine(model, optimiser, train_dataloader, val_dataloader, training_name='-1805-refine-1-4' , n_epochs=5000, patience=100, decoys=1)
